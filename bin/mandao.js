...
// ─────────────────────────────────────────────
// MCP HTTP 客户端（POST + Mcp-Session-Id；Node.js >= 20 fetch）
// ─────────────────────────────────────────────
...
/**
 * 调用 MCP tools/call（与 https://agent.hinadt.com/mcp 约定一致）。
 *
 * POST initialize（响应头 Mcp-Session-Id）→ notifications/initialized → tools/call；
 * tools/call 的响应体可能是 application/json 或 text/event-stream（SSE）。
 *
...
async function callMcpTool(toolName, toolArgs, cfg, timeout = 30000) {
  const endpointUrl = (cfg.url || DEFAULT_URL).replace(/\/$/, "");
  ...
  const postJsonHeaders = (sessionId) => {
    const h = {
  ...
  async function postJson(body, sessionId) {
    return fetch(endpointUrl, {
      method: "POST",
      headers: postJsonHeaders(sessionId),
  ...
  try {
    let res = await postJson(
      {
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: "mandao-cli", version: PKG.version || "1.0.0" },
        },
      },
      undefined,
    );
    ...
    if (!res.ok) {
      ...
      throw new Error(`MCP initialize 失败 HTTP ${res.status}: ${text}`);
    }

    const sessionId = res.headers.get("mcp-session-id");
    if (!sessionId) {
      throw new Error("MCP initialize 未返回 Mcp-Session-Id 响应头");
    }

    const initBody = await res.json();
    if (initBody.error) {
      throw new Error(`initialize 失败: ${initBody.error.message}`);
    }
    ...
    if (!res.ok && res.status !== 202) {
      const text = await res.text().catch(() => "");
      throw new Error(
        `notifications/initialized 失败 HTTP ${res.status}: ${text}`,
      );
    }
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error(`tools/call 失败 HTTP ${res.status}: ${text}`);
    }
    ...
    if (ctype.includes("application/json")) {
      ...
      if (msg.id !== TOOL_ID) {
        throw new Error("tools/call 返回了意外 JSON-RPC id");
      }
      if (msg.error) {
    ...
    if (!res.body) throw new Error("tools/call 响应无可读流");
    ...
    while (true) {
      ...
      if (done) throw new Error("tools/call SSE 流意外结束");
    ...
    if (err.cause) {
      ...
      throw new Error(
        `${err.message}（原因: ${cause.code ?? cause.message ?? cause}）`,
      );
    }
    throw err;
  }
...
function printQjda(d) {
  ...
  console.log("\n  ┌ 逾期概况（近6个月）");
  console.log(
    `  │  逾期机构数: ${r.member_count ?? "-"}   逾期订单数: ${r.order_count ?? "-"}   逾期总金额: ${r.debt_amount ?? "-"} 元`,
  );
  ...
  console.log("\n  ├ 近1月共债概况");
  console.log(
    `  │  机构数:${r.current_org_count || "-"}  订单数:${r.current_order_count || "-"}  已还款:${r.current_order_amt || "-"}  订单金额:${r.current_order_lend_amt || "-"}`,
  );
...
function printZxRadarV2(d) {
  ...
  console.log("\n  ┌ 申请雷达报告详情");
  console.log(
    `  │  申请准入分:${ar.A22160001 ?? "-"}  置信度:${ar.A22160002 ?? "-"}  命中机构:${ar.A22160003 ?? "-"}`,
  );
  console.log(
    `  │  申请命中消金类机构数:${ar.A22160004 ?? "-"}  申请命中网络贷款类机构数:${ar.A22160005 ?? "-"}  机构总查询次数:${ar.A22160006 ?? "-"}  最近一次查询时间:${ar.A22160007 ?? "-"}`,
  );
  console.log(
    `  │  查询次数 近1月:${ar.A22160008 ?? "-"}  近3月:${ar.A22160009 ?? "-"}  近6月:${ar.A22160010 ?? "-"}`,
  );

  console.log("\n  ├ 行为雷达报告详情");
  console.log(
    `  │  贷款行为分:${br.B22170001 ?? "-"}  置信度:${br.B22170051 ?? "-"}  正常还款率:${br.B22170034 ?? "-"}%`,
  );
  console.log(
    `  │  贷款笔数 近1月:${br.B22170002 ?? "-"}  近3月:${br.B22170003 ?? "-"}  近6月:${br.B22170004 ?? "-"}  近12月:${br.B22170005 ?? "-"}`,
  );
  console.log(
    `  │  逾期笔数(M0+) 近6月:${br.B22170025 ?? "-"}  近12月:${br.B22170026 ?? "-"}`,
  );
  console.log(
    `  │  逾期笔数(M1+) 近6月:${br.B22170028 ?? "-"}  近12月:${br.B22170029 ?? "-"}`,
  );
  console.log(
    `  │  失败扣款 近1月:${br.B22170035 ?? "-"}  近3月:${br.B22170036 ?? "-"}  近6月:${br.B22170037 ?? "-"}`,
  );

  console.log("\n  └ 信用现状报告详情");
  console.log(
    `     网贷建议额度:${cr.C22180001 ?? "-"}元  置信度:${cr.C22180002 ?? "-"}  在贷机构:${cr.C22180003 ?? "-"}`,
  );
  console.log(
    `     消金建议额度:${cr.C22180011 ?? "-"}元  置信度:${cr.C22180012 ?? "-"}  在贷机构:${cr.C22180007 ?? "-"}`,
  );
}
...
function printQjtz(d) {
  ...
  console.log("\n  ┌ 结果详情");
  console.log(
    `  │  最大逾期金额:${r.max_overdue_amt ?? "-"}  最长逾期天数:${r.max_overdue_days ?? "-"}  最近逾期时间:${r.latest_overdue_time ?? "-"}`,
  );
  console.log(
    `  └  最大履约金额:${r.max_performance_amt ?? "-"}  最近履约时间:${r.latest_performance_time ?? "-"}  履约笔数:${r.count_performance ?? "-"}`,
  );
  console.log(
    `  └  当前逾期机构数:${r.currently_overdue ?? "-"}  当前履约机构数:${r.currently_performance ?? "-"}  异常还款机构数:${r.acc_exc ?? "-"}  睡眠机构数:${r.acc_sleep ?? "-"}`,
  );
}
...
function printFmlh(d) {
  ...
  console.log("\n  ┌ 结果详情");
  console.log(
    `  │  最大逾期金额:${r.max_overdue_amt ?? "-"}  最长逾期天数:${r.max_overdue_days ?? "-"}  最近逾期:${r.latest_overdue_time ?? "-"}`,
  );
  console.log(
    `  └  当前逾期机构:${r.currently_overdue ?? "-"}  当前履约机构:${r.currently_performance ?? "-"}  异常还款:${r.acc_exc ?? "-"}  睡眠机构:${r.acc_sleep ?? "-"}`,
  );
}
...
const PRODUCTS = {
  qjda: {
    tool: "getRiskXQjdaV130",
    title: "风控-全景指数 QJDA 1.3.0",
    print: printQjda,
  },
  zxradarv2: {
    tool: "getRiskZxRadarV2",
    title: "风控-综合指数V2 ZX-RadarV2_1 2.1.0",
    print: printZxRadarV2,
  },
  qjtz: {
    tool: "getRiskQjtzV140",
    title: "风控-信用探查指数 QJTZ 1.4.0",
    print: printQjtz,
  },
  fmlh: {
    tool: "getRiskFmlhV140",
    title: "风控-履约指数 FMLH 1.4.0",
    print: printFmlh,
  },
};
...
function cmdInit(args) {
  if (!args.authorization) {
    ...
    console.error(
      '      mandao init --authorization "YOUR_API_KEY"  （会自动补充 Bearer 前缀）',
    );
    process.exit(1);
  }
  // 自动补充 Bearer 前缀
...
function cmdConfigShow() {
  ...
  if (!cfg) {
    console.log(
      '尚未初始化，请先执行：mandao init --authorization "Bearer YOUR_API_KEY"',
    );
    return;
  }
...
async function cmdQuery(product, args) {
  ...
  if (!def) {
    console.error(
      `错误：未知产品 "${product}"，可用产品：${Object.keys(PRODUCTS).join(" / ")}`,
    );
    process.exit(1);
  }
  if (!args.idNo || !args.idName) {
    ...
    console.error(
      `示例：mandao query ${product} --idNo 110101199001011234 --idName 张三`,
    );
    process.exit(1);
  }
  ...
  switch (cmd) {
    ...
    case "config":
      if (args._[0] === "show") cmdConfigShow();
        ...
        console.error("用法：mandao config show");
        process.exit(1);
      }
      break;
    ...
    case "query": {
      ...
      if (!product) {
        console.error(
          `错误：请指定产品，可用：${Object.keys(PRODUCTS).join(" / ")}`,
        );
        process.exit(1);
      }
