...
if [[ "$SCRIPT_SOURCE" == /dev/fd/* ]] || [[ "$SCRIPT_SOURCE" == /proc/*/fd/* ]] || [ ! -f "$SCRIPT_SOURCE" ]; then
  ...
  MANDAO_INSTALL_TAR_URL="${MANDAO_INSTALL_TAR_URL:-https://github.com/Hinadt-Inc/financial-services-mandao/archive/refs/heads/main.tar.gz}"
...
cat > "$MCP_CONFIG_DEST" << MCPJSONEOF
{
  "mcpServers": {
    "mandao-company": {
      ...
      "description": "漫道MCP - 全景指数/综合指数V2/信用探查指数/履约指数"
    }
  }
}
MCPJSONEOF
...
echo ""
echo "1. Verify MCP: Claude should list tools from mandao-company (e.g. getRiskFmlhV140)."
echo ""
